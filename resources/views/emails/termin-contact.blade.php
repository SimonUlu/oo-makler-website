...
<div class="container">
    <div class="content">
        ...
        <p><strong>Telefon:</strong> {{ $formData['reachable'] ?? 'nicht angegeben' }}</p>
        <p><strong>Nachricht:</strong> {{ $formData['message'] ?? 'nicht angegeben' }}</p>
        </br>
        </br>
        <p><strong>Verknüpfung zu Nutzer mit Datensatznummer:</strong> {{ $formData['addressId'] ?? 'nicht angegeben' }}</p>

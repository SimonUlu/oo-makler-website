...
<form id="contact-form" action="{{ url('contact-form') }}" method="POST">
        ...
        <div class="flex flex-col items-center justify-center hidden sm:flex sm:col-span-2" x-data="{ logoUrl: '{{ $logoUrl }}' }">
            ...
            <div class="mt-4">
                @if (isset($estate['userPhoto']))
                    <img class="max-w-[120px] rounded-full" src="data:image/jpeg;base64, {{ $estate['userPhoto'] }}"
                        alt="Ihr Kontakt">
                @else
                    <!-- Display default photo if user photo does not exist -->
                    <img class="max-w-[120px] rounded-full" src="{{ $logoUrl }}" alt="Ihr Kontakt">
                @endif
            </div>
    ...
    <div class="pb-3 pl-3 pr-3 border-b border-l border-r border-gray-200 rounded-b-lg">
        <h3 class="py-3 ml-2 text-base font-bold tracking-tight text-gray-900 sm:text-base">
            Kontaktformular</h3>
        <div class="grid px-2 py-4 gap-y-2">
            ...
            <div>
                <x-elements.textarea name="message" label="Nachricht">
                    Bitte kontaktieren Sie mich zum Verkauf meiner Immobilie.
                </x-elements.textarea>
            </div>

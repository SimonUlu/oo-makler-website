<div class="relative w-full bg-primary-100 mt-10 mb-12">
    <div class="grid grid-cols-2 pt-12 m-auto md:space-x-4">
        <div class="col-span-2 min-h-[450px] sm:min-h-[500px] md:min-h-[550px] lg:min-h-[600px]">
...
<script src="/js/polygonCoordinates/polygonCoordinates.js"></script>
...
<script>
    ...
    console.log(geoJsonObj.type);
    var districtJson = {{ stadtteile_json }};
    console.log(districtJson);

    // Extrahiere die Postleitzahlen aus der plzList und erstelle ein Array mit diesen Postleitzahlen
    var plzArray = districtJson.map(function(item) {
        return item.plz;
    });
    // console.log(plzArray);
    ...
    var filteredFeatures = geoJsonObj.features.filter(function(feature) {
        var plz_code = feature.properties.plz_code;
    ...
    map.on('load', function() {
        ...
        districtJson.forEach(function(entry) {
            ...
            map.on('mousemove', 'postalCodeAreas', function(e) {
                ...
                if (featuresUnderMouse.length > 0) {
                    ...
                    // Ändern Sie nur das Cursor-Styling, wenn featurePlzId einer Ihrer ausgewählten IDs entspricht

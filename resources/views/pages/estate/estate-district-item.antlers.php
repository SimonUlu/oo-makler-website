    ...
    <div class="grid w-full max-w-lg grid-cols-1 md:flex-col md:mb-0 md:space-x-0 md:max-w-none">
        ...
        <!-- End IMage Slider -->
            ...
            <div class="relative px-5 pt-2 pb-2 border rounded-b-lg md:pt-0">
                <h3
                    class="flex justify-between mb-2.5 text-lg font-bold text-gray-900 md:mt-4 "
                >
                    <span>
                        {{ if elements.vermarktungsart == "miete" }}
                            {{ elements.warmmiete | round | european_number:0 }} €
                        {{ else }}
                            {{ elements.kaufpreis | round | european_number:0 }} €
                        {{ /if }}
                    </span>
                    {{ if elements.vermarktungsart == "miete" }}
                ...
                <div class="space-y-2 text-md">
                    <div class="flex space-x-2 text-gray-700 dark:text-gray-400">
                        <span>{{ elements.wohnflaeche }} m&sup2;</span>
                        <span class="ml-2"> · {{ elements.anzahl_zimmer | round }} Zimmer </span>
                        <span class="ml-2 hidden sm:flex"> · {{ elements.baujahr }} (Baujahr) </span>
                    </div>
                    <div class="flex space-x-2 text-gray-500 text-sm sm:text-base dark:text-gray-400">
                        <span class="capitalize">
                            {{elements.objektart | ucfirst}}
                        </span>
                        <span>in</span>
                        <span class="ml-2">
                            {{elements.ort| truncate:23:...}}
                        </span>
                    </div>
                </div>
                ...
                <div class="flex flex-col items-center justify-center hidden sm:flex sm:col-span-2" x-data="{ logoUrl: '{{ $logoUrl }}' }">
                    <div class="flex items-center justify-between w-full"> <!-- Add 'items-center' class here -->
                        ...
                        <a href="/immobilien/details/{{elements.Id}}" class="inline-flex justify-center items-center py-2.5 px-5 text-sm font-medium text-center md:w-auto lg:col-span-12 focus:ring-4 focus:outline-none" target="_blank">Ansehen<span aria-hidden="true">&nbsp;→</span></a>
                    </div>
                </div>
            </div>
            <!-- Ende Neue Section -->
